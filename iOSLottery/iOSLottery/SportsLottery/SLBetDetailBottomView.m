...
#import "SLBetDetailChuanGuanView.h"
...
@interface SLBetDetailBottomView ()<UITextFieldDelegate,CLRequestCallBackDelegate>
...
@property (nonatomic, strong) SLBetDetailChuanGuanView *chuanGuanSelectView;//串关选择View
...
@implementation SLBetDetailBottomView
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        self.multipleTextField.text = [NSString stringWithFormat:@"%zi", [SLBetInfoManager getDefaultMultiple]];
...
- (void)requestFinished:(CLBaseRequest *)request
{
    if (request.urlResponse.success) {
        ...
        //无数据的时候，本地计算预计奖金
        [self setBonusAmount:[SLBetDetailDataManager getExpectedBonus]];
        return;
    }
...
- (void)reloadBetDetailBottonViewUI{
    ...
    [self createUIWithSelect:self.chuanGuanButton.selected];
    self.multipleTextField.text = [NSString stringWithFormat:@"%zi", [SLBetInfoManager getMultiple]];
    [self reloadAwardAndNote];
...
- (void)reloadSelectLabel{
    ...
    NSString *chuanGuan = @"";
    for (SLChuanGuanModel *model in [SLBetDetailDataManager getChuanGuan]) {
...
- (void)reloadAwardAndNote{
    ...
    if ([SLBetDetailDataManager getNote] > 0) {
        //如果注数大于0
        self.selectBetTypeLabel.hidden = YES;
        self.noteMultipleLabel.hidden = NO;
        self.awardLabel.hidden = NO;
        
        NSString *noteAward = [NSString stringWithFormat:@"%zi注 %zi倍 共%zi元", [SLBetDetailDataManager getNote], [SLBetDetailDataManager getMultiple], [SLBetDetailDataManager getMultiple] * [SLBetDetailDataManager getNote] * 2];
        ;
        ...
        self.request.lotteryNumber = [SLBetDetailDataManager getAllOddsString];
        self.request.betTimes = [NSString stringWithFormat:@"%ld",[SLBetDetailDataManager getMultiple]];
        
        //异常情况，不请求
        if (self.request.lotteryNumber.length == 0) return;
...
//        [self.request cancel];
    ...
    if (([SLBetDetailDataManager getMultiple] * [SLBetDetailDataManager getNote]) > 0) {
...
- (void)payButtonOnClick:(UIButton *)btn{
    ...
    //点击了支付按钮
    if (([SLBetDetailDataManager getMultiple] * [SLBetDetailDataManager getNote]) <= 0) return;
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string{
    ...
    BOOL ret = [textField sl_limitNumberMax:[SLBetInfoCache shareBetInfoCache].maxTimes ShouldChangeCharactersInRange:range replacementString:string];
    
    if (ret) {
        [SLBetInfoCache shareBetInfoCache].allSelectBetItem.betMultiple = [[textField.text stringByReplacingCharactersInRange:range withString:string] integerValue];
    }else{
        [SLBetInfoCache shareBetInfoCache].allSelectBetItem.betMultiple = [textField.text integerValue];
    }
    [self reloadSelectLabel];
    [self reloadAwardAndNote];
    return ret;
...
- (void)textFieldDidEndEditing:(UITextField *)textField{
    ...
    if (!(textField.text && textField.text.length > 0)) {
        ...
        textField.text = @"1";
        [SLBetInfoCache shareBetInfoCache].allSelectBetItem.betMultiple = 1;
        [self reloadSelectLabel];
        [self reloadAwardAndNote];
    }
}
...
- (SLBetDetailChuanGuanView *)chuanGuanSelectView{
    
    if (!_chuanGuanSelectView) {
        WS_SL(_weak)
        _chuanGuanSelectView = [[SLBetDetailChuanGuanView alloc] init];
