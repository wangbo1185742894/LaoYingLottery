...
@interface BBDrawNoticeResultView ()
...
@implementation BBDrawNoticeResultView
...
- (void)setDataWithArray:(NSArray *)dataArray isCancel:(NSInteger)isCancel;
{
    ...
    //数据校验，数组长度不等于5代表数据错误
    if (!(dataArray && dataArray.count == 4)){
    ...
    BBDrawNoticeResultItem *lastItem;
    
    for (int i = 0; i < dataArray.count; i++) {
        
        BBDrawNoticeResultItem *item = [[BBDrawNoticeResultItem alloc] initWithFrame:(CGRectZero)];
...
@implementation BBDrawNoticeResultItem
...
- (void)addConstraints
{
    ...
    [self.playLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.width.mas_equalTo(SL__SCALE(94.f));
...
- (void)setUpItemDataWithString:(NSString *)str
{
    ...
    if (arr && arr.count == 2) { //数据正确
        ...
        self.playLabel.text = [arr[0] stringByReplacingOccurrencesOfString:@"让分" withString:@""];
