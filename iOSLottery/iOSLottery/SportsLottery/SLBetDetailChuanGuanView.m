...
@interface SLBetDetailChuanGuanView ()
...
@implementation SLBetDetailChuanGuanView
...
- (void)setChoosableChuanGuan{
    ...
    NSArray *chuanGuanArray = [SLBetDetailDataManager getChuanGuan];
...
- (void)setButton:(UIButton *)btn select:(BOOL)select{
    ...
    //记录选择的串关
    if (btn.selected) {
        [SLBetInfoManager saveSelecrChuanGuan:[NSString stringWithFormat:@"%zi", btn.tag]];
    }else{
        [SLBetInfoManager removeSelecrChuanGuan:[NSString stringWithFormat:@"%zi", btn.tag]];
    }
...
- (UILabel *)chuanGuanExplainLabel{
    ...
    if (!_chuanGuanExplainLabel) {
        ...
        _chuanGuanExplainLabel.text = @"比分、总进球和半全场最高支持4串1";
