//
//  CLModifyLoginPwdViewController.m
//  iOSLottery
//
...
@interface CLModifyLoginPwdViewController () <CLRequestParamSource,CLRequestCallBackDelegate>
...
@property (nonatomic, strong) CLModifyLoginPwdAPI* modifyPwdAPI;

@end

@implementation CLModifyLoginPwdViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navTitleText = @"修改登录密码";
...
- (NSDictionary *)paramsForPostApi:(CLBaseRequest *)request {
    ...
    return @{@"password":self.oldPwd.text.md5,
             @"new_password":self.reNewPwd.text.md5,
             @"re_new_password":self.reNewPwd.text.md5};
}
...
- (CQInputTextFieldView *)oldPwd
{
    if (!_oldPwd) {
        ...
        _oldPwd.limitType = CQInputLimitTypeNumberAndEnglish;
        _oldPwd.limitLength = 15;
...
- (CQInputTextFieldView *)reNewPwd
{
    if (!_reNewPwd) {
        ...
        _reNewPwd.textPlaceholder = @"请输入6-15位数字或字母";
        _reNewPwd.inputTextType = inputTextTypeUnderline;
        _reNewPwd.limitType = CQInputLimitTypeNumberAndEnglish;
...
- (CQInputTextFieldView *)commitPwd
{
    if (!_commitPwd) {
        ...
        _commitPwd.limitType = CQInputLimitTypeNumberAndEnglish;
...
- (CLModifyLoginPwdAPI *)modifyPwdAPI {
    
    if (!_modifyPwdAPI) {
        _modifyPwdAPI = [[CLModifyLoginPwdAPI alloc] init];
