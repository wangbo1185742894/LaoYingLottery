...
@implementation CLSSQNormalBetView
...
#pragma mark - 配置默认数据
- (void)assignDefaultData:(CLSSQNormalBetTerm *)betTerm{
...
- (void)ssq_randomSelectBall{
    ...
    [self.randomAnimationArray addObjectsFromArray:[self.redBallView randomSelectNumberWithArray:[CLTools randomArrayWithCount:6 maxNumber:33]]];
    [self.randomAnimationArray addObjectsFromArray:[self.blueBallView randomSelectNumberWithArray:[CLTools randomArrayWithCount:1 maxNumber:16]]];
...
- (void)hiddenOmission:(BOOL)hidden{
    ...
    [self.redBallView mas_remakeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo([self.redBallView getOneGroupBallVeiwHeightWithCount:33]);
    }];
    
    [self.blueBallView mas_remakeConstraints:^(MASConstraintMaker *make) {
        
        make.top.equalTo(self.infoLabel.mas_bottom).offset(__SCALE(45.f) + [self.redBallView getOneGroupBallVeiwHeightWithCount:33]);
        make.left.right.equalTo(self);
        make.height.mas_equalTo([self.blueBallView getOneGroupBallVeiwHeightWithCount:16]);
...
- (UILabel *)infoLabel{
    ...
    if (!_infoLabel) {
        ...
        _infoLabel.text = @"请至少选择6个红球和1个蓝球";
    }
    return _infoLabel;
}
- (CLOneGroupBallView *)redBallView{
    ...
    WS(_weakSelf)
    if (!_redBallView) {
        _redBallView = [[CLOneGroupBallView alloc] initWithFrame:CGRectZero ballCount:33 ballColor:UIColorFromRGB(0xd90000)];
...
- (CLOneGroupBallView *)blueBallView{
    ...
    WS(_weakSelf)
    if (!_blueBallView) {
        _blueBallView = [[CLOneGroupBallView alloc] initWithFrame:CGRectZero ballCount:16 ballColor:UIColorFromRGB(0x295fcc)];
...
- (CLSSQNormalBetTerm *)betTerm{
    
    if (!_betTerm) {
        _betTerm = [[CLSSQNormalBetTerm alloc] init];
    }
    return _betTerm;
}
