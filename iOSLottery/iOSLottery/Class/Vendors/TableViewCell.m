...
//点击举报
- (void)didClickedReportButton {
    if (self.clickedReportButtonCallback) {
        self.clickedReportButtonCallback(self);
        [self.menu menuHide];
    }
}
//点赞
...
- (Menu *)menu {
    ...
    [_menu.commentButton addTarget:self action:@selector(didClickedCommentButton)
                  forControlEvents:UIControlEventTouchUpInside];
    [_menu.reportButton addTarget:self action:@selector(didClickedReportButton)
                 forControlEvents:UIControlEventTouchUpInside];
