...
@property (nonatomic, assign) BOOL isClearButton;//是否是清空
@end

@implementation CLFTBetFooterView
...
- (void)assignBetNote:(NSInteger)note minBonus:(NSInteger)minBonus maxBonus:(NSInteger)maxBonus hasSelectBetButton:(BOOL)hasSelectBetButton playMothed:(NSInteger)playMothed{
    ...
    AttributedTextParams *params1 = [AttributedTextParams attributeRange:NSMakeRange(noteRange.location + 1, noteStr.length - noteRange.location - 1) Color:UIColorFromRGB(0xffff00)];
    ...
    if (minBonus == maxBonus) {
        ...
        AttributedTextParams *params2 = [AttributedTextParams attributeRange:NSMakeRange(minBonusRange.location, awardStr.length - minBonusRange.location) Color:UIColorFromRGB(0xffff00)];
        ...
        AttributedTextParams *params2 = [AttributedTextParams attributeRange:minBonusRange Color:UIColorFromRGB(0xffff00)];
        AttributedTextParams *params3 = [AttributedTextParams attributeRange:NSMakeRange(maxBonusRange.location + 1, awardStr.length - maxBonusRange.location - 1) Color:UIColorFromRGB(0xffff00)];
        [self.awardInfoLabel attributeWithText:awardStr controParams:@[params2, params3]];
    }
    if (playMothed > 4 && !hasSelectBetButton) {
        //表示胆拖 玩法 不显示机选
...
- (UILabel *)midLabel{
    ...
    if (!_midLabel) {
        ...
        _midLabel.textColor = UIColorFromRGB(0xffffff);
...
- (UILabel *)awardInfoLabel{
    ...
    if (!_awardInfoLabel) {
        ...
        _awardInfoLabel.textColor = UIColorFromRGB(0xffffff);
