...
- (void) show {
    ...
    [self.buttonItems enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        UIAlertActionStyle actionStype;
        if (self.isDefaultButtonStyle) {
            actionStype = UIAlertActionStyleDefault;
        }else{
            actionStype = (self.cancelButtonIndex == idx)?UIAlertActionStyleCancel:((self.destructiveButtonIndex == idx)?UIAlertActionStyleDestructive:UIAlertActionStyleDefault);
        }
...
- (void)hidden{
    ...
    if (self.cAlertViewController.isViewLoaded && self.cAlertViewController.view.window) {
        [self.cAlertViewController dismissViewControllerAnimated:NO completion:nil];
    }
}
