...
@interface CLDEBetDetailViewController ()<UITableViewDelegate, UITableViewDataSource, CLRequestCallBackDelegate, CLCreateOrderDelegate, CLAlertControllerDelegate>

@property (nonatomic, strong) CLDElevenBetHeaderView *betDetailHeaderView;//头部视图
...
@implementation CLDEBetDetailViewController
...
//    NSLog(@"%@", OrderInfo);
    ...
    if ([retInfo[@"isBetAvailable"] integerValue] == 0) {
        ...
        self.endBetGuideView.desText = [retInfo[@"message"] stringByReplacingOccurrencesOfString:@"_" withString:@""];
        WS(_weakSelf)
...
- (void)requestFinished:(CLBaseRequest *)request{
    ...
    if (self.request == request) {
        ...
        [self.betDetailHeaderView reloadDataForBetHeaderView];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    WS(_weakSelf)
    static NSString *cellId = @"DEBetDetailTableViewCell";
    CLDEBetDetailTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellId];
    if (!cell) {
        cell = [[CLDEBetDetailTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellId];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [CLDElevenViewController presentFastThreeViewControllerWithInitialVC:self selectedIndex:(self.dataSourceArray.count - indexPath.row - 1) isSelectBetInfo:YES gameEn:self.lotteryGameEn completion:^{
...
- (void)configConstraint{
    ...
    [self.betDetailHeaderView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.equalTo(self.view);
        make.top.equalTo(self.view).offset(0.f);
    ...
    [self.topView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(self.betDetailHeaderView.mas_bottom);
    }];
...
- (void)getData{
    ...
    if (tempArray.count < [dataManager getBetDetailModelWithGameEn:self.lotteryGameEn].count) {
...
- (void)assignFooterViewData{
    ...
    [self.detailFooterView assignBetNote:[[CLNewLotteryBetInfo shareLotteryBetInfo] getAllNoteWithLottery:self.lotteryGameEn] period:[[CLNewLotteryBetInfo shareLotteryBetInfo] getPeriodWithLottery:self.lotteryGameEn] multiple:[[CLNewLotteryBetInfo shareLotteryBetInfo] getMultipleWithLottery:self.lotteryGameEn]];
}
...
- (void)textFieldChange{
    ...
    [[CLNewLotteryBetInfo shareLotteryBetInfo] setPeriod:[self.chaseMutipleView.chasePeriod integerValue] lottery:self.lotteryGameEn];
     [self.detailFooterView assignBetNote:[[CLNewLotteryBetInfo shareLotteryBetInfo] getAllNoteWithLottery:self.lotteryGameEn] period:[self.chaseMutipleView.chasePeriod integerValue] multiple:[self.chaseMutipleView.chaseMultiple integerValue]];
}
#pragma mark - 点击了蒙版
