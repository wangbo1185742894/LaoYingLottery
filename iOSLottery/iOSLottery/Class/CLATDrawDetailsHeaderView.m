...
@interface  CLATDrawDetailsHeaderView()
...
@implementation CLATDrawDetailsHeaderView
...
- (void)setData:(CLAwardVoModel *)data
{
    ...
    if ([data.gameEn isEqualToString:@"pl3"] || [data.gameEn isEqualToString:@"fc3d"]) {
        
        [self.periodTitle mas_updateConstraints:^(MASConstraintMaker *make) {
           
            make.centerX.equalTo(self);
        }];
        
        self.bonusScale.hidden = YES;
        self.bonusTitle.hidden = YES;
