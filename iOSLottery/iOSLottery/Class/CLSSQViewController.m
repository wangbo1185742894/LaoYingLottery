...
@interface CLSSQViewController ()<CLLotteryBetViewDelegate, CLRequestCallBackDelegate, UITableViewDelegate, UITableViewDataSource>
//ui
...
@property (nonatomic, strong) CLSSQNormalBetView *normalView;
@property (nonatomic, strong) CLSSQDTBetView *danTuoView;
@property (nonatomic, strong) CLSSQFooterView *footerView;//底部视图
...
@implementation CLSSQViewController
...
- (void)viewDidLoad {
    ...
    if (!kDevice_Is_iPhoneX) {
        ...
        [CLNewbieGuidanceService showNewGuidanceInWindowWithType:CLNewbieGuidanceTypeSSQ];
    }
...
+ (void)presentSSQViewControllerWithInitialVC:(UIViewController *__weak)initial selectedIndex:(NSInteger)index isSelectBetInfo:(BOOL)isSelect gameEn:(NSString *)gameEn completion:(void (^)(void))completion {
    CLSSQViewController *ssq = [[self alloc] init];
    ssq.selectedIndex = index;
    ssq.isSelectedBetInfo = isSelect;
    ssq.gameEn = gameEn;
    [initial presentViewController:[[CLBaseNavigationViewController alloc] initWithRootViewController:ssq] animated:YES completion:completion];
}
...
- (void)getCacheData{
    ...
    if (mainBetModel.missingNumbers && [mainBetModel.missingNumbers isKindOfClass:[NSDictionary class]]) {
        if (self.playMothedType == CLSSQPlayMothedTypeNormal) {
            [self.normalView assignOmissionDataWithRed:mainBetModel.missingNumbers[@"RED"] blue:mainBetModel.missingNumbers[@"BLUE"]];
        }else{
            [self.danTuoView assignOmissionDataWithRed:mainBetModel.missingNumbers[@"RED"] blue:mainBetModel.missingNumbers[@"BLUE"]];
        }
    }else{
        if (self.playMothedType == CLSSQPlayMothedTypeNormal) {
...
- (void)configDefaultData{
    ...
    if (self.playMothedType == CLSSQPlayMothedTypeNormal) {
        [self.normalView assignDefaultData:(CLSSQNormalBetTerm *)betTerm];
    }else{
        [self.danTuoView assginDefaultData:(CLSSQDTBetTerm *)betTerm];
    }
}
- (void)configRequest{
...
- (void)ssqConfirmBetTerm{
    ...
    //如果有投注项  跳转投注详情
    [[CLAllJumpManager shareAllJumpManager] open:[NSString stringWithFormat:@"CLSSQBetDetailViewController_push/%@", self.gameEn] dissmissPresent:YES];
}
#pragma mark - 胆拖投注是否允许投注
- (BOOL)getDTAllowBet{
    
    return ((self.danTuoView.betTerm.redDanArray.count + self.danTuoView.betTerm.redTuoArray.count >= 7) && self.danTuoView.betTerm.betNote > 0);
}
...
- (void)setPlayMothedType:(CLSSQPlayMothedType)playMothedType{
    ...
    self.navigationView.navigationTitle = (_playMothedType == CLSSQPlayMothedTypeNormal) ? @"双色球" : @"双色球胆拖";
...
- (void)setIsHiddenOmission:(BOOL)isHiddenOmission{
    ...
    if (!([[CLCacheManager getCacheFormLocationcacheFileWithName:ssqOmissionPrompt] integerValue] == 1) && isHiddenOmission == NO) {
        //展示说明
        [CLLotteryOmissionView showLotteryOmissionInWindowWithType:CLOmissionPromptTypeD11];
        [CLCacheManager saveToCacheWithContent:@(1) cacheFileName:ssqOmissionPrompt];
    }
...
- (CLNavigationView *)navigationView{
    if (!_navigationView) {
        ...
        _navigationView.navigationTitle = @"双色球";
        WS(_weakSelf)
...
//主视图
- (CLSSQNormalBetView *)normalView{
    
    WS(_weakSelf)
    if (!_normalView) {
        _normalView = [[CLSSQNormalBetView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, __SCALE(493.f))];
//        CGSize size = [_normalView systemLayoutSizeFittingSize:UILayoutFittingCompressedSize];
//        _normalView.frame = CGRectMake(0, 0, size.width, size.height);
...
- (CLSSQDTBetView *)danTuoView{
    
    WS(_weakSelf)
    if (!_danTuoView) {
        _danTuoView = [[CLSSQDTBetView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, __SCALE(798.f))];
//        CGSize size = [_danTuoView systemLayoutSizeFittingSize:UILayoutFittingCompressedSize];
//        _danTuoView = [[CLSSQDTBetView alloc] initWithFrame:CGRectMake(0, 0, size.width, size.height)];
...
- (UITableView *)mainTableView{
    ...
    if (!_mainTableView) {
        ...
        _mainTableView.backgroundColor = UIColorFromRGB(0xeeeee5);
...
- (CLSSQAwardHeaderView *)awardHeaderView{
    ...
    if (!_awardHeaderView) {
        ...
        _awardHeaderView.backgroundColor = UIColorFromRGB(0xeeeee5);
    }
