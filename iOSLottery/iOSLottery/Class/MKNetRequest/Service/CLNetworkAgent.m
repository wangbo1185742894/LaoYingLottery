...
+ (void)launchRequest:(CLBaseRequest*)request params:(NSDictionary*)apiParams apiUrl:(NSString*)url success:(CLCallback)success fail:(CLCallback)fail {
    ...
    if (apiParams && (apiParams.count > 0) && (requestType == CLRequestTypePOST)) {
        if (request.baseUrlConfig && [request.baseUrlConfig respondsToSelector:@selector(signRequestapiClient:requestType:Params:)]) {
            [request.baseUrlConfig signRequestapiClient:apiClient requestType:requestType Params:apiParams];
        }
    }
    
//    NSLog(@"all headers : %@",[apiClient.requestSerializer HTTPRequestHeaders]);
    //配置请求BaseUrl
    NSString* baseUrl = @"";
